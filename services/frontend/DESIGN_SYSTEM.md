...
- **History and refusals (23.09, later)**: `HistoryList.vue` rows (time + source · «до» chip name → «после» chip name · one-line digest or error · status pill `ok` / `fail` / `warn` («не подходит») / `run` with a pulsing dot); used in full on the «История» screen and limited to five under the upload panel. A failed run is a `.fail` panel with a 4 px left border: red for pipeline errors, amber (`--c-dup`, `--c-dup-bg`, `--c-loss-bg` added to tokens) for documents that were refused, with the input files as chips and the offending ones highlighted.
