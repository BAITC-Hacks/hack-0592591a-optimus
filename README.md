...
С настроенной моделью — итог `passed=36 failed=0`, 3–6 минут (выполняется полный демо-анализ через `POST /api/analyses/demo`): состояние сервисов, извлечение PDF и отказ неверного типа, регистрация и вход, демо-анализ до статуса `done`, контрольный набор по [scripts/verify-control.py](scripts/verify-control.py) (`units ДИТААД and ДОА created`, `ДНМ and ДККМ kept`, `audit direction reorganized into ДИТААД and ДОА`, `POTENTIAL_LOSS at до п. 5.6.2 with its quote`, `POTENTIAL_DUPLICATION после п. 5.4.3 / 5.5.8`, `POTENTIAL_CONFLICT cites после п. 5.5.2`, `every finding has a verified quote`, `every unit points to the correct document and side`, `finding counts reconcile with actual records`, `conclusion covers all findings with valid document-qualified references`, `O1: legislation check ran on 477 norms…`), отказ инструкции по сборке мебели с `irrelevant_document`, история пользователя, ответы `415` / `422` / `404` на неверный ввод.

Без ключа (`.env.example` как есть) обычный запуск даёт `passed=19 failed=0`: вместо демо-анализа проверяется ответ `503 llm_unavailable`; `--require-analysis` в этом случае завершается ошибкой. Проверки аутентификации используют синтетический аккаунт `smoke@example.com` / `smoke-pass-123`. При другом порте: `WEB_PORT=3100 ./scripts/smoke.sh`; для развёрнутой версии: `BASE_URL=https://hackalem-ai-wg.germanywestcentral.cloudapp.azure.com ./scripts/smoke.sh --require-analysis`.
