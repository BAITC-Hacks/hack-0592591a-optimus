...
- [scripts/smoke.sh](scripts/smoke.sh): тринадцать HTTP-проверок — статус API и доступность MongoDB через `/api/health`, главная страница, ошибка для неизвестного маршрута API, извлечение пунктов из PDF и два отказа на неверный ввод, регистрация, вход, `/api/auth/me` с cookie сессии и три отказа аутентификации (неверный пароль, неверные данные регистрации, запрос без сессии).
...
5. Пользователь регистрируется (имя, email, пароль не короче 8 символов) или входит. Backend проверяет поля, при регистрации хеширует пароль `scrypt` и записывает пользователя в коллекцию `users` (email уникален), затем ставит httpOnly-cookie с JWT. Страница показывает имя пользователя; «Выйти» очищает cookie.
...
8. Проверяющий запускает smoke-скрипт и получает результаты тринадцати HTTP-проверок.
...
Ожидаемый результат: тринадцать успешных проверок (`backend health`, `mongo reachable`, `frontend is up`, `unknown api route`, `extract pdf clauses`, `reject unsupported type`, `reject missing file`, `signup (or already registered)`, `login`, `reject wrong password`, `reject invalid signup`, `me requires session`, `me with session cookie`), итог `passed=13 failed=0` и код завершения `0`. Проверки аутентификации используют синтетический аккаунт `smoke@example.com` с паролем `smoke-pass-123`: первый запуск регистрирует его, последующие получают `email_taken`, что также считается успехом. Скрипт ищет `"status":"ok"` и `"db":"ok"` в ответе `/api/health`, заголовок страницы в ответе `/`, код `not_found` для `/api/does-not-exist`, ссылку `"п. 2, стр. 1, строка 2"` в ответе на PDF, который он сам генерирует, код `unsupported_format` для `.txt` и HTTP `400` для запроса без файла. Анализ оргструктуры и AI он не проверяет, потому что их пока нет.
...
Датасетов, демонстрационных записей, загрузчиков данных и seed-команд в репозитории нет. MongoDB запускается без прикладных данных; backend при старте создаёт в базе `hackalem` уникальный индекс по `users.email`, записи в `users` появляются при регистрации. Демонстрационных аккаунтов нет: регистрируйтесь любым email, подтверждение почты не требуется. Вызовов внешних API и интеграций с AI-провайдерами нет; соответствующие переменные окружения только зарезервированы.
...
- Smoke-проверка охватывает тринадцать HTTP-запросов, включая доступность MongoDB, извлечение текста из PDF, регистрацию и вход. Сравнение документов, анализ и сохранение прикладных данных она не проверяет, потому что их пока нет.
...
Для запуска текущей заготовки не нужны аккаунты команды, платные подписки или API-ключи. Нужны доступ к репозиторию и возможность скачать контейнерные образы. Для входа в интерфейс достаточно зарегистрировать любой аккаунт на стартовой странице. Доступ к будущим AI-функциям пока не настроен, поскольку сами функции отсутствуют.
