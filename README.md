...
- По адресу `http://localhost:3000/#demo` доступен отдельный демонстрационный концепт интерфейса аналитика (боковая панель сессий, шесть шагов от загрузки до заключения, проверка замечаний с цитатами внутри полного текста документа, печать отчёта в PDF). Он работает **целиком на вшитых демонстрационных данных** (`services/frontend/src/data/demo_data.json`), не вызывает API анализа и не является частью основного сценария; заглушки помечены `[ДЕМО]`. Основной рабочий интерфейс — на главной странице.
