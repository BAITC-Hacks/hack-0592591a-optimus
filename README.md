...
- По адресу `http://localhost:3000/#demo` доступен отдельный демонстрационный концепт интерфейса аналитика: четыре шага (документы → анализ → проверка → заключение), проверка замечаний с цитатами в модальном окне полного текста документа, печать отчёта в PDF. Он работает **целиком на вшитых демонстрационных данных** (`services/frontend/src/data/demo_data.json`), не вызывает API анализа и не является частью основного сценария. Основной рабочий интерфейс — на главной странице.
