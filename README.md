...
- **Конвейер анализа** ([services/backend/src/pipeline](services/backend/src/pipeline)), пять этапов, как в docs/TASK.md §2–§5:
  ...
  5. [report.js](services/backend/src/pipeline/report.js) — код составляет русское заключение из проверенных наблюдений и источников подразделений: все находки, цитаты, ссылки с `doc_id`, исходные имена файлов, рекомендации по проверке и дисклеймер. Дополнительного вызова модели нет. Неподтверждённый источник останавливает заключение с `report_unverified`; произвольный текст модели не переносится в отчёт.
...
- **Frontend** ([services/frontend](services/frontend)): Vue 3 + Vite, статика через Caddy. После входа демо-аккаунтом пользователь загружает комплекты «до»/«после», видит этапы анализа, изменения подразделений, возможные потери, дублирование и конфликты. Из каждого наблюдения можно открыть исходный пункт; заключение и рекомендации доступны для просмотра и скачивания. История показывает запуски текущего пользователя. Контрольный анализ через API остаётся доступен по `POST /api/analyses/demo`.
...
На контрольной паре анализ занимает примерно 3–6 минут и делает около 60–80 обращений к модели (оба документа по 25 страниц, ~450 пунктов и около 250–300 функций на сторону). Загруженные файлы обрабатываются в памяти и на диск не пишутся; в базе остаются пункты (текст и ссылки), подразделения, функции, сопоставления, выводы и заключение.
...
Конвейер внутри backend (docs/TASK.md §2): `clauses.js` (extractor + код) → `structure.js` (модель + код) → `extract.js` (модель) → `compare.js` (код + модель-судья) → `report.js` (код). Принцип: extractor режет, код сопоставляет и считает, модель размечает и оценивает кандидатов, код строит заключение; каждая цитата проверяется кодом до выдачи.
...
- `DEMO_USER_EMAIL`, `DEMO_USER_PASSWORD`, `DEMO_USER_NAME` — демо-аккаунт, создаётся при старте, если его нет; пустые значения заменяются демонстрационными значениями Compose.
...
Ответ — `{"analysis_id":"a_…"}`. Подставьте id и повторяйте запрос, пока `status` не станет `done` (примерно 3–6 минут):
...
Проверка из чистого клона (ключ передаётся в отдельном приватном env-файле, не в `.env.example`):

```bash
./scripts/clean-test.sh --env-file /absolute/path/reviewer.env
```

Скрипт клонирует закоммиченное состояние, собирает без кеша, запускает изолированную MongoDB на отдельных портах `3900`/`3901` и требует успешного основного сценария. Ожидаемая последняя строка: `>> Clean-clone main-scenario test passed`. Без настроенной модели полный тест не проходит. Для отдельной проверки запуска без ключа: `./scripts/clean-test.sh --infrastructure-only`; она явно не подтверждает работу AI-анализа. Файл конфигурации читается Compose как данные, не исполняется и не выводится в журнал.
...
- Одно лучшее соответствие на функцию: распределение составной обязанности между несколькими пунктами может быть отмечено как частичная потеря. Сопоставление и поиск дублирования используют эвристики и ограниченный набор кандидатов; полнота на произвольных документах не измерена. Правило конфликта проверяет исполнение аудита и контроль его качества, а не все возможные виды конфликта интересов.
- Неполная оценка кандидатов конфликта оставляет предупреждение `unreviewed`; она не означает отсутствия конфликта. Приложения после оглавления сохраняются для анализа.
...
- Анализ занимает примерно 3–6 минут на пару документов по 25 страниц; для больших комплектов время зависит от числа пунктов и пар кандидатов. Кэша ответов модели нет.
...
Для запуска нужны только репозиторий и Docker. Для основного сценария (анализ) нужен ключ OpenAI-совместимого API с доступом к чат-модели в режиме JSON: впишите в `.env` значения `OPENAI_API_KEY` и `LLM_MODEL` (команда проверяла на `gpt-5.6-terra`); `EMBEDDING_MODEL=text-embedding-3-small` необязателен, но включает семантический подбор кандидатов. Ключ команды в репозитории отсутствует; приватный способ передачи отдельного ограниченного ключа жюри ещё должен быть подтверждён командой. До этого требование проверки без аккаунтов команды не закрыто. Без ключа доступны интерфейс, извлечение текста, аутентификация и проверки инфраструктуры. `smoke.sh --require-analysis` в этом случае завершается с ошибкой.
...
Вызовы модели: один модуль, таймаут 90 с, два повтора с задержкой на сетевые ошибки и 429/5xx, не больше 12 параллельных вызовов, режим JSON, проверка каждого ответа zod-схемой с одним повторным запросом при невалидном ответе; отказ провайдера (401/403/404/5xx) превращается в понятную ошибку анализа. Текст документов передаётся модели как данные внутри блока `<document>` с указанием не выполнять содержащиеся в нём инструкции; модель возвращает только идентификаторы пунктов и метки, цитаты берутся из текста пункта кодом и проверяются как подстрока. Заключение строится кодом из записей с повторно проверенными источниками, без свободного текста модели. Ключи читаются только на сервере и в бандл frontend не попадают.
