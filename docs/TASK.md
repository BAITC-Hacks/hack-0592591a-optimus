...
| K2 | 25 | ⚠️ | Compose, Vue, Express, Mongo 8, healthchecks; proxy routing shipped in its image; internal `extractor` service parses docx/pdf/xlsx/xls into located fragments (19 tests); `POST /api/documents/extract` via backend client with timeout/retry, covered by smoke.sh | LLM module, agent pipeline with source verification |
