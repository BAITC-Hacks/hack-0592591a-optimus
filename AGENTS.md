...
| K5 | Potential and originality | 10 | Built today, but only on top of a working K1–K4 (`docs/TASK.md` §12a): O1 (regulatory check, using the regulatory corpus tool in §7a) and O3 (redistribution recommendations). O2 (benchmarking) is not built: there is no benchmark data. Architecture that takes new document types or rule sets without a rewrite. |

**Per-change rule (every commit, every agent):**
1. Before starting: `git fetch && git pull --rebase` (§10). Read the tail of `docs/CHANGELOG.md`, and claim the chunk there with a `start` line (§10). Then name which criterion (K1–K5) or Must-have (M1–M5) the change moves. If none, say so under §3 before writing code.
...
3. In the same commit, append a `done` line to `docs/CHANGELOG.md`, and update the Criteria scorecard in `docs/TASK.md` if a status changed. Only mark ✅ after seeing it work in Docker (§3). If README claims changed, update the README too (§9).
...
**Dataset definition.** The dataset is taken from adilet.zan.kz by scraping, and only acts in force are used (payload `status` = `действует`). The export filter enforces this. Acts that have lost force (`утратил силу`) never enter `services/backend/data/regulatory/`.
...
3. **The data ships in the repo** as `services/backend/data/regulatory/<doc_id>.jsonl`: one raw payload per line, in `chunk_index` order, unchanged. Pick acts by the control set and keep the total under about 10 MB.
4. **Backend usage (implemented).**
   - **Loading.** The Dockerfile copies `data/` into the image. At startup, `ensureRegulatoryNorms()` in `src/seed.js` runs next to `ensureDemoUser()`. It calls `src/regulatoryNorms.js`, which validates every line (status `действует`, adilet `source_url`; a bad line is reported with its file and line) and upserts into Mongo `regulatory_norms`. There is a unique index on `doc_id` + `chunk_index`, and acts no longer shipped are deleted. Each piece gets `norm_id` `{doc_id}#{chunk_index}` and `clause` `Статья {article}` (plus `, п. {point}`); `breadcrumb` and `source_url` are kept.
   - **Matching.** `src/pipeline/regulatory.js` runs from `run.js` inside the compare stage. Code picks ≤ 3 candidates per «после» function, by cosine when `EMBEDDING_MODEL` is set (norm vectors cached on the Mongo rows, keyed by model and text hash), else by Jaccard ≥ 0.05. The LLM judges `basis | contradicts | none` (`prompts/regulatory.md`). Code rejects an unknown norm id or confidence < 0.6, quotes the norm line closest to the function, and verifies both quotes.
   - **Output.** The result is `analysis.regulatory` (`REGULATORY_BASIS`, `POTENTIAL_REGULATORY_CONFLICT`), plus `stats.regulatory_*` and a code-built section above the conclusion's disclaimer. Any error gives `regulatory: []` and `regulatory_status: "failed"`, never a failed analysis.
   - **Tests.** `test/regulatory.test.js` needs no key. Like every backend test, it runs with `MONGO_URL` set, which compose does.
5. **"Открыть на adilet.zan.kz" button (required in the frontend; implemented in `services/frontend/src/components/RegulatoryTab.vue`, the «Нормативные требования» tab, with the link logic in `src/adilet.js`: `ADILET_BASE`, `adiletHref`).** Every norm shown in the UI gets a button that opens that clause on adilet in a new tab (`target="_blank" rel="noopener noreferrer"`). It follows the design system (§6a): `.btn .btn-ghost .btn-sm`, an inline action rather than the screen's one CTA, with an `external-link` icon added to `Icon.vue`. Next to it, show `ред. от {redaction_date}`, because the site may already carry a newer edition. Code builds the link from the stored `source_url`, never from LLM output. It is rendered only when the path matches `^/(rus|kaz)/docs/[A-Za-z0-9_]+(#z\d+)?$`. The host comes from one frontend constant, `ADILET_BASE`, set to `https://old.adilet.zan.kz`. Checked on 23.09.2026: the new adilet.zan.kz is a test version that shows a version pop-up and "Не удалось отобразить страницу", while `https://old.adilet.zan.kz/rus/docs/Z1500000410#z37` scrolls to the clause. Re-check before 17:40, and switch the constant if the new site works. The quote shown in the app remains the evidence. The button is a supplement: a finding must stay verifiable even when adilet is down.
...
mkdir -p services/backend/data/regulatory
for id in Z030000415_ Z1500000410 Z1200000550; do export_act "$id" > "services/backend/data/regulatory/$id.jsonl"; done
wc -l services/backend/data/regulatory/*.jsonl   # line counts must match the Pieces column
```
...
- **Changelog: the team's shared state.** `docs/CHANGELOG.md` is an append-only log of what is done, claimed and blocked, across all three people and their agents. Its format is at the top of the file.
  - **Before a chunk:** read the tail (`tail -25 docs/CHANGELOG.md`) and check that nobody holds a `start` on the same area. Then append your own `start` line, and commit and push that line alone at once. That push is the claim.
  - **With the work:** append the `done` line in the same commit.
  - **Stuck:** append `blocked` and the reason. **Giving up:** append `drop`.
  - **Stale claims:** a `start` with no `done` for 45 minutes is stale. Ask that person; do not take the work over silently.
  - **Never edit or reorder old lines.** `.gitattributes` sets `merge=union` on this file, so concurrent appends never conflict on rebase.
